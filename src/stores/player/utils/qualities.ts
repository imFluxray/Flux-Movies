...
export type LoadableSource = {
  /** "embed" is a third-party player page, never handed to a <video> */
...
type BaseSource =
  ...
  | {
      /** a third-party player page, driven over postMessage */
      type: "embed";
      url: string;
      headers?: Stream["headers"];
      preferredHeaders?: Stream["preferredHeaders"];
...
export type ExternalAudioVariant = {
  id: string;
  label: string;
  language: string;
  source: BaseSource;
...
/** Intro/outro markers, in seconds, when a provider supplies them. */
export interface SourceSkipSegment {
  start: number;
  end: number;
}

export type SourceSliceSource = BaseSource & {
  audioVariantId?: string;
  audioVariants?: ExternalAudioVariant[];
  skips?: {
    intro?: SourceSkipSegment;
    outro?: SourceSkipSegment;
  };
};
...
export function selectQuality(
  ...
  if (source.type === "hls" || source.type === "embed")
    return {
      stream: source,
      quality: null,
    };
