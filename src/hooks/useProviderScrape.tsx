...
import { getFluxProviders } from "@/backend/providers/fluxProviders";
import {
  FluxInitEvent,
  runFluxProviders,
} from "@/backend/providers/fluxRunner";
...
function useBaseScrape() {
  ...
  const initEvent = useCallback((evt: FluxInitEvent) => {
    setSources(
      evt.sourceIds
        .map((v) => {
          // Sources from providers.json are not in the library's metadata
          // table, so init carries their display names alongside the ids.
          const source = getCachedMetadata().find((s) => s.id === v);
          const name = source?.name ?? evt.sourceNames?.[v];
          if (!name) throw new Error("invalid source id");
...
export function useScrape() {
  ...
  const startScraping = useCallback(
    async (media: ScrapeMedia) => {
      // providers.json is the source of truth. The scrapers compiled into
      // @movie-web/providers are all dead upstream, so they are only reached
      // when no runtime config is present - that keeps a typo in the config
      // from taking the player down entirely.
      const fluxProviders = await getFluxProviders();
      if (fluxProviders.length > 0) {
        startScrape();
        const ordered =
          enableSourceOrder && preferredSourceOrder.length
            ? [...fluxProviders].sort((a, b) => {
                const ai = preferredSourceOrder.indexOf(a.id);
                const bi = preferredSourceOrder.indexOf(b.id);
                // ids absent from the preference keep their file order, last
                return (
            ...
            : fluxProviders;
        const fluxOutput = await runFluxProviders({
          media,
          providers: ordered,
          events: {
            init: initEvent,
