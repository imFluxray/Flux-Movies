...
export function LinksDropdown(props: { children: React.ReactNode }) {
  ...
  const fluxProfile = useFluxAccountStore((s) => s.profile);
        ...
        <div className="rounded-lg absolute w-64 bg-dropdown-altBackground top-full mt-3 right-0">
          {fluxProfile ? (
            <DropdownLink className="text-white" href="/settings">
              <UserAvatar />@{fluxProfile.username}
            </DropdownLink>
          ) : (
            <DropdownLink href="/settings" icon={Icons.RISING_STAR} highlight>
              {t("navigation.menu.register")}
            </DropdownLink>
          )}
          <Divider />
          <DropdownLink href="/profiles" icon={Icons.USER}>
            Profiles
          </DropdownLink>
          ...
          <DropdownLink href="/taste" icon={Icons.WAND}>
            Taste Finder
          </DropdownLink>
          <DropdownLink href="/parties" icon={Icons.WATCH_PARTY}>
            Watch Parties
          </DropdownLink>
          {deviceName ? (
            <DropdownLink
