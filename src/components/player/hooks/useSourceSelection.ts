...
export function useSourceScraping(sourceId: string | null, routerId: string) {
  ...
  const [request, run] = useAsyncFn(async () => {
    ...
    const runtimeProvider = (await getFluxProviders()).find(
      (provider) => provider.id === sourceId,
    );
    if (runtimeProvider) {
      const output = await runFluxProviders({
        media: scrapeMedia,
        providers: [runtimeProvider],
      });
      if (!output) throw new Error("No playable stream from this source");
      setCaption(null);
      setSource(
        convertRunoutputToSource(output),
        convertProviderCaption(output.stream.captions ?? []),
        progress,
      );
  ...
  }, [sourceId, meta, progress, router, setCaption, setSource, setSourceId]);
