...
import { banCodecFromUrl } from "@/utils/codecBans";
import {
  canChangeVolume,
  canFullscreen,
...
export function makeVideoElementDisplayInterface(): DisplayInterface {
    ...
    if (src.type === "hls") {
      ...
      if (!hls) {
        ...
        hls.on(Hls.Events.ERROR, (event, data) => {
          ...
          // A mirror whose audio the browser cannot decode is finished for
          // this device. Remember it so the next resolve skips it entirely,
          // whichever code path picked it.
          if (
            ...
            const url =
              (data.frag as any)?.url ?? (data.frag as any)?._url ?? src?.url;
            const banned = banCodecFromUrl(url);
            if (banned.length)
              console.warn(
                `[flux] undecodable audio; banning ${banned.join(", ")}`,
              );
          }
  ...
  function setSource() {
    ...
    // Picture-in-picture shows only the video element, so DOM-rendered
    // subtitles vanish there. Switching to a native <track> is the same trick
    // already used for iOS fullscreen, and is what makes captions survive PiP.
    ...
    videoElement.addEventListener("webkitpresentationmodechanged", () => {
      ...
      if (mode === "picture-in-picture") emit("needstrack", true);
      else if (!isFullscreen) emit("needstrack", false);
    });
