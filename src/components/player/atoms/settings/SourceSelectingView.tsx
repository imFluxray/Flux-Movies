...
export function EmbedSelectionView({ sourceId, id }: EmbedSelectionViewProps) {
  ...
  const sourceName = useMemo(() => {
    if (!sourceId) return "...";
    const runtimeSource = getCachedFluxProviders().find(
      (source) => source.id === sourceId,
    );
    if (runtimeSource) return runtimeSource.name;
...
export function SourceSelectionView({
  ...
  const runtimeSources = useAsync(getFluxProviders, []);
  const sources = useMemo(() => {
    if (!metaType) return [];
    return (runtimeSources.value ?? []).filter(
      (source) => !source.media_types || source.media_types.includes(metaType),
    );
  }, [metaType, runtimeSources.value]);
