...
import { isAnimeSourceId } from "@/backend/providers/fluxProviders";
import { takeNextFluxOutput } from "@/backend/providers/fluxRunner";
...
export function RealPlayerView() {
  ...
  const hasAnimeAudioVariants = usePlayerStore((s) =>
    Boolean(s.source?.audioVariants?.length),
  );
  ...
  // Keep the rest of the concurrent race warm. A hard playback failure uses
  // the next successful provider immediately; continuous buffering gets 20s.
  useEffect(() => {
    if (
      status !== playerStatus.PLAYBACK_ERROR ||
      hasAnimeAudioVariants ||
      isAnimeSourceId(currentSourceId)
    )
      return undefined;
    let attempts = 0;
    const timer = window.setInterval(() => {
      attempts += 1;
      const fallback = takeNextFluxOutput(currentSourceId);
      if (fallback) {
        window.clearInterval(timer);
        playAfterScrape(fallback);
      } else if (attempts >= 20) {
        window.clearInterval(timer);
      }
    }, 500);
    return () => window.clearInterval(timer);
  }, [currentSourceId, hasAnimeAudioVariants, playAfterScrape, status]);
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      const fallback = takeNextFluxOutput(currentSourceId);
      if (fallback) playAfterScrape(fallback);
    }, 20_000);
