...
export function ScrapingPart(props: ScrapingProps) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!isMounted()) return;
      // Playback is the primary outcome. Never commit the not-found page or
      // run optional telemetry before handing a successful stream to the
      // player: the source-status snapshot can still be one React render
      // behind when a fast provider resolves.
      if (output) props.onGetStream?.(output);
      else
        props.onResult?.(
      ...
      try {
        ...
        // Anonymous metrics must never block playback.
      }
