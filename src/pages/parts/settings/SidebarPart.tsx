...
import { useFluxAccountStore } from "@/stores/fluxAccount";
...
export function SidebarPart() {
  ...
  // The sudo-flix backend account and the Flux account are different systems.
  // Signing into Flux never populates the former, which is why this panel used
  // to insist you were logged out while you were plainly logged in.
  const fluxProfile = useFluxAccountStore((state) => state.profile);
  ...
  const settingLinks = [
    ...
    {
      textKey: "settings.sources.title",
      id: "settings-sources",
      icon: Icons.CASTING,
    },
    {
      ...
      >
        ...
        >
          <div className="px-3 py-3.5 rounded-lg bg-largeCard-background bg-opacity-50 grid grid-cols-2 gap-4">
            ...
            {/* Backend URL - Flux does not use a sudo-flix backend, so this row
                only appears for someone who has deliberately configured one.
                Otherwise it rendered a permanent "—" next to an "insecure"
                badge, which read as a fault rather than as "not applicable". */}
            {backendUrl ? (
              <div className="col-span-2 space-y-1">
            ...
            {/* Flux account */}
            <div className="col-span-2 space-y-1">
              <p className="text-type-dimmed font-medium">Flux account</p>
              {fluxProfile ? (
                <p className="text-white flex items-center gap-2">
                  <span>@{fluxProfile.username}</span>
                  <span className="text-type-dimmed px-2 py-0.5 rounded bg-settings-sidebar-badge text-xs">
                    {fluxProfile.hasLicence ? "Licensed" : "Free"}
                  </span>
                </p>
              ) : (
                <p className="text-type-dimmed">Not linked</p>
              )}
            </div>

            {/* Backend user id, when a backend is in use */}
            {backendUrl && account?.userId ? (
              <div className="col-span-2 space-y-1">
                <p className="text-type-dimmed font-medium">
                  {t("settings.sidebar.info.userId")}
                </p>
                <p className="text-white">{account.userId}</p>
              </div>
            ) : null}
